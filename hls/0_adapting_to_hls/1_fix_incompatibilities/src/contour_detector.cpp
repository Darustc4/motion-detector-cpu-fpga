...
namespace motdet
{
    namespace imgutil
    {
         ...
         */
        void follow_border(Image<uint8_t, motdet_total> &in, uint16_t width, Motion_detector::Contour &found_contour, const uint16_t i, const uint16_t j, uint16_t i2, uint16_t j2);
        ...
        void contour_detection(Image<uint8_t, motdet_total> &conts_image, Contour_package &conts, bool trim_borders)
        {
            ...
            for(uint16_t i = 1; i < motdet_height-1; ++i)
            {
                for(uint16_t j = 1; j < motdet_width-1; ++j)
                {
                    ...
                    Motion_detector::Contour &found_contour = conts.contours[conts.contour_count];

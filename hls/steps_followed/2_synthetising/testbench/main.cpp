...
int main(int argc, char** argv)
{
    ...
    Detection contours; // Storage for detected movement across frames.
    ...
    while(true)
    {
        ...
        Image<uint16_t, motdet::original_total> image_in(motdet::original_width);
        for(uint32_t i = 0; i < motdet::original_total; ++i)
        {
            uint32_t mapped = i*3;
            image_in[i] = rgb_data[mapped] * 76.245 + rgb_data[mapped+1] * 149.685 + rgb_data[mapped+2] * 29.07;
        }

        Detection detected_result;
        detect_motion(image_in, detected_result); // Submit frame to FPGA for processing.

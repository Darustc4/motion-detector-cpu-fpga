...
namespace motdet
{
    namespace imgutil
    {
        ...
        void downsample(hls::stream<uint16_t, MOTDET_STREAM_DEPTH> &in, hls::stream<uint16_t, MOTDET_STREAM_DEPTH> &out)
        {
            ...
            {
                uint8_t submit_line_check = (i+1) % motdet_reduction_factor;
                if(submit_line_check)
                {
                    // No line to output, keep filling the buffer
                    for(uint16_t motdet_j = 0; motdet_j < motdet_width; ++motdet_j)
                    {
                        buffer[motdet_j] = in.read();
                        for(uint8_t red_i = 1; red_i < motdet_reduction_factor; ++red_i) buffer[motdet_j] += in.read();
                    }
                }
                else
                {
                    ...
                    for(uint16_t motdet_j = 0; motdet_j < motdet_width; ++motdet_j)
                    {
                        uint32_t total_val = in.read();
                        for(uint8_t red_i = 1; red_i < motdet_reduction_factor; ++red_i)
                        {
                            total_val += buffer[motdet_j] + in.read();
                        }
                        out.write(total_val/squared_red_factor);
                    }
                }
            }
